;(function () {
    ...
    if (ajaxForm_list.length) {
        Wind.use('ajaxForm', 'artDialog', function () {
            ...
            $('button.J_ajax_submit_btn').click(function (e) {
                e.preventDefault();
...
function jumpurl(referer,info){
    Wind.use('ajaxForm', 'artDialog', function () {
        ...
        if (referer) {
            ...
            if(window.parent.art){
                //iframe弹出页
                window.parent.location.href = referer;
            }else{
...
function lalert($money, $wallet, $discount, $total, $wait_money, $yes_money, $yes_money_total, $pay_status, $ordertype) {
    if ($ordertype == 2) {
        if ($pay_status == 1) {
            $content = "已预付：" + $yes_money + "元</br>";
            $content += "订单总计：" + $total + "元</br>";
            $content += "优惠券抵扣：" + $discount + "元</br>";
            $content += "钱包抵扣：" + $wallet + "元</br>";
            $content += "还需支付：" + $wait_money + "元</br>";
            $content += "实际支付：" + $money + "元";
        } else if ($pay_status == 0) {
            if ($yes_money_total <= $yes_money) {
                $content = "已预付：" + $yes_money + "元</br>";
                $content += "订单总计：" + $total + "元</br>";
                $content += "优惠券抵扣：" + $discount + "元</br>";
                $content += "钱包抵扣：" + $wallet + "元</br>";
                $content += "还需支付：" + parseFloat($wait_money - $discount) + "元";
            } else {
                $content = "已预付：" + $yes_money + "元</br>";
                $content += "订单总计：" + $total + "元</br>";
                $content += "优惠券抵扣：" + $discount + "元</br>";
                $content += "钱包抵扣：" + $wallet + "元</br>";
                $content += "还需支付：" + $wait_money + "元";
            }
