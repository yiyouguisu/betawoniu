...
class OrderController extends CommonController
{
    ...
    public function ordershow()
    {
      ...
      if ($orderid == '') {
          ...
          $data = M('order a')
            ->join("left join zz_order_time c on a.orderid=c.orderid")
            ->join("left join zz_refund_apply d on a.orderid=d.orderid")
          ...
          $booinfo = D('book_room')->where(['orderid' => $orderid])->find();

          //edit by meroc @2016-10-10
          if(!empty($type)) {
            $data['productinfo']['vouchersrange'] = $data['productinfo']['vouchersdiscount'] = 0;
          }
          if ($booinfo) {
              $hinfo = D('hostel')->find($booinfo['hid']);
              if ($hinfo) {
                  $data['productinfo']['vouchersrange'] = $hinfo['vouchersrange'];
                  $data['productinfo']['vouchersdiscount'] = $hinfo['vouchersdiscount'];
              }
          }
          if ($data) {
              exit(json_encode(array('code' => 200, 'msg' => "加载成功", 'data' => $data)));
    ...
    public function evaluate()
    {
        ...
        if (empty($user)) {
          ...
          $room = M('book_room a')
            ->join("left join zz_room b on a.rid=b.id")
            ->where(array('a.orderid' => $orderid))
            ->find();
    ...
    {
        $ret = file_get_contents("php://input");//$GLOBALS['HTTP_RAW_POST_DATA']
        $ret = json_decode($ret, true);
        $uid = intval(trim($ret['uid']));
        $orderid = trim($ret['orderid']);
