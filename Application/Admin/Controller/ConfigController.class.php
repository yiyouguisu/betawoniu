...
class ConfigController extends CommonController {
    ...
    public function version(){
        if (IS_POST) {
            if(empty($_POST['member_anzhuo_version'])&&empty($_POST['member_ios_version'])){
                $this->error("版本号不能为空!");
            }
            ...
            $member_anzhuo=M('version')->where('type=1')->order(array('inputtime'=>'desc'))->find();
            $this->assign('member_anzhuo', $member_anzhuo);
            $member_ios=M('version')->where('type=2')->order(array('inputtime'=>'desc'))->find();
    ...
    /**
     ...
     public function smstemplate() {
         if (IS_POST) {
             ...
             $config = D("Config")->where('groupid=9')->select();
