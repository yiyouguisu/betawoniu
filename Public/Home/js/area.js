...
function GetAreaSelect(provice,city,district,pid,cid,did){
	...
	o.bind = function(){
		$(o.provice).unbind().change(function(){
			pid = $(o.provice).val();
			alert(pid)
			o.changed = 'p';
			//加载中...
			...
			$(o.city).attr("disabled",true);
			$(o.district).attr("disabled",true);
			//加载中...
			o.getAlist(pid);
		});
		$(o.city).unbind().change(function(){
			cid = $(o.city).val();
			o.changed = 'c';
			//加载中...
			$(o.city).removeAttr("disabled");
			$(o.district).empty();
			$("<option value=''>加载中...</option>\r\n").appendTo(o.district);
			$(o.district).attr("disabled",true);
			//加载中...
		...
		if(o.defaultPid>0){
			o.changed = 'dp';
			o.getAlist(0);//初始化的时候省份上级ID默认为1,因为是调用省份本身，上一级是中国
		}else{
			o.changed = 'dp';
			o.getAlist(0);//初始化的时候省份上级ID默认为1,因为是调用省份本身，上一级是中国
		}
	...
	o.displayA=function(d){
		...
		if(o.changed=='dp'){
			...
			$(o.city).empty();
			if(d.NoCity==1) d.option = "<option value=''>--请先选择省份--</option>\r\n";
			else d.option = "<option value=''>--请选择城市--</option>\r\n"+d.option;
			...
			//加载中...
			$(o.district).removeAttr("disabled");
			//加载中...
			$(o.district).empty();
			if(d.NoCity==1) d.option = "<option value=''>--请先选择省份--</option>\r\n";
			else d.option = "<option value=''>--请选择城市--</option>\r\n"+d.option;
