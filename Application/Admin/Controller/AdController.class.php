...
class AdController extends CommonController {
    ...
    public function index() {
        ...
        foreach ($data as $k => $r) {
            ...
            $data[$k]["catname"] = D("adtype")->where("id=" . $r["catid"])->getField("catname");
        }
        $show = $page->show();
    ...
    public function edit() {
        if ($_POST) {
            ...
            $this->assign("category", $select_categorys);
            $this->assign("data", $data);
            $hostel=M('hostel')->where(array('status'=>2,'isdel'=>0))->order(array('listorder'=>'desc','id'=>'desc'))->select();
            $this->assign("hostel", $hostel);
            $note=M('note')->where(array('status'=>2,'isdel'=>0))->order(array('listorder'=>'desc','id'=>'desc'))->select();
            $this->assign("note", $note);
            $party=M('activity')->where(array('status'=>2,'isdel'=>0))->order(array('listorder'=>'desc','id'=>'desc'))->select();
