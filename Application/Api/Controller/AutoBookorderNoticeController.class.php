...
class AutoBookorderNoticeController extends CommonController {
	...
	public function bookordernotice(){
        $strOut="";
        $sql="SELECT orderid,uid FROM(SELECT a.orderid,a.uid, (a.yes_money_total - a.yes_money) AS wait_yes_money FROM zz_order a LEFT JOIN zz_order_productinfo b ON a.orderid = b.orderid LEFT JOIN zz_order_time c ON a.orderid = c.orderid LEFT JOIN zz_product d ON b.pid = d.id WHERE(c.pay_status = 0)AND (a.ordertype = 2)AND (d.selltime > " . strtotime("-5 minute") . ")AND(d.selltime <= " . time() . ")) AS A WHERE(wait_yes_money <= 0)";
        $data=M()->query($sql);
