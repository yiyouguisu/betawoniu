                ...
                <table width="100%" cellspacing="0">
                    ...
                    <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td align="center" > 
                                ...
                                <?php if(authcheck('Admin/Order/party')): ?><a href="<?php echo U('Admin/Order/party',array('aid'=>$vo['id']));?>" >报名详情</a> 
                                    <?php else: ?>
                                    <font color="#cccccc">报名详情</font><?php endif; ?> 
                                <?php if(($vo['status']) == "1"): if(authcheck('Admin/Party/review')): ?><a href="javascript:;" onClick="omnipotent('selectid','<?php echo U('Admin/Party/review',array('id'=>$vo['id']));?>','审核',1,700,300)">审核</a>
                                      <?php else: ?>
                                      <font color="#cccccc">审核</font><?php endif; endif; ?>
                                <?php if(($vo['isoff']) == "0"): if(authcheck('Admin/Party/setoff')): ?><a href="<?php echo U('Admin/Party/setoff',array('id'=>$vo['id'],'isoff'=>1));?>" >下架 </a>
                                        <?php else: ?>
                                        <font color="#cccccc">下架</font><?php endif; ?>  
                                <?php else: ?>
                                    <?php if(authcheck('Admin/Party/setoff')): ?><a href="<?php echo U('Admin/Party/setoff',array('id'=>$vo['id'],'isoff'=>0));?>" >启用 </a>
                                        <?php else: ?>
                ...
                <div class="btn_wrap_pd">
                    <label class="mr20"><input type="checkbox" class="J_check_all" data-direction="y" data-checklist="J_check_y">全选</label>   
                     <?php if(authcheck('Admin/Party/listorder')): ?><button class="btn btn_submit mr10 " type="submit" name="submit" value="listorder" >排序</button><?php endif; ?> 
                         <?php if(authcheck('Admin/Party/jinpins')): ?><button class="btn btn_submit mr10 " type="submit" name="submit" value="jinpins">精品</button><?php endif; ?>
                      <?php if(authcheck('Admin/Party/unjinpins')): ?><button class="btn btn_submit mr10 " type="submit" name="submit" value="unjinpins">取消精品</button><?php endif; ?>
                    <?php if(authcheck('Admin/Party/pushs')): ?><button class="btn btn_submit mr10 " type="submit" name="submit" value="pushs">推荐</button><?php endif; ?>
                      <?php if(authcheck('Admin/Party/unpushs')): ?><button class="btn btn_submit mr10 " type="submit" name="submit" value="unpushs">取消推荐</button><?php endif; ?>
