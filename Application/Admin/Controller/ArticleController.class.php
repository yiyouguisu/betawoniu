...
class ArticleController extends CommonController {
    ...
    public function index() {
        ...
        foreach ($data as $k => $r) {
            ...
            $news=M('article')->where(array('isdel'=>0,'parentid'=>$r['id']))->order(array('id'=>'desc'))->select();
            $data[$k]["news"]=$news;
        }
        $show = $page->show();
    ...
    public function edit() {
        if ($_POST) {
            ...
            if($data['parentid']!=0){
                $data['parenttitle']=M('article')->where(array('id'=>$data['parentid']))->getField("title");
            }
            $this->assign("data", $data);
            $this->display();
        }
    ...
    public function add() {
        if ($_POST) {
            ...
            $id= I('get.id', null, 'intval');
            $this->assign("id", $id);
            $data['parenttitle']=M('article')->where(array('id'=>$id))->getField("title");
            $this->assign("data", $data);
            $this->display();
        }
    }
    ...
    public function delete() {
        $id = $_GET['id'];
        $did=M("Article")->where(array('id'=>$id))->save(array('isdel'=>1,'deletetime'=>time()));
        if ($did) {
            $this->success("删除内容成功！");
        } else {
            $this->error("删除内容失败！");
    ...
    public function del() {
        if (IS_POST) {
            ...
            foreach ($_POST['ids'] as $id) {
                M("Article")->where(array('id'=>$id))->save(array('isdel'=>1,'deletetime'=>time()));
            }
            $this->success("删除成功！");
        } else {
            $this->error("删除失败！");
