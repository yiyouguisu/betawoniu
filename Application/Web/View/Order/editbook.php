...
<body class="back-f1f1f1">
    ...
    <div class="container padding_0" style="margin-top:6rem">
        <div class="act_e" style="margin-bottom:2rem;">
            <div class="act_e1 fl">
                <img src="{$data.room_img}">
            </div>
            <div class="act_e2 fr">
                <div class="act_e3">{$data.hotel_name}</div>
                <div class="act_e4">{$data.room_name}</div>
            </div>
        </div>
        <form action="{:U('Web/Order/editconfirm')}" method="post" id='form' onsubmit="return checkForm();">
            ...
            <div class="yr">
                ...
                <div class="yr-b" style="margin-bottom:0;">
                    <div class="yr-c center fl time-box" data-target="#start_time" data-type="start">
                        ...
                        <div class="yr-c3">
                            <input class="ggo_text begin" name="starttime" type="date" value="{$data.starttime|date='Y-m-d',###}" id="start_time" style="display:none;">
                            <span>{$data.starttime|date='Y-m-d', ###}</span>
                        </div>
                    </div>
                    <div class="yr-d fl pr">
                        共<span id='day'>{$data.days}</span>天
                        <div class="yr_line pa"></div>
                        <input type='hidden' value="{$data.days}" name="days" class='day' />
                    </div>
                    <div class="yr-c center fl time-box" data-target="#leave_time" data-type="end">
                        ...
                        <div class="yr-c3">
                            <input class="ggo_text end" name="endtime" type="date" value="{$data.endtime|date='Y-m-d', ###}"
                            id="leave_time" style="display:none;">
                            <span>{$data.endtime|date='Y-m-d', ###}</span>
                        </div>
                    </div>
                </div>
            ...
            <div class="we_a" style="margin-bottom:8rem;">
                <div class="yr_list">
                    ...
                    <div class="we_b">
                        ...
                        <div class="we_b2 center ft12">
                            <input type="number" name='rooms' id="rooms" class="we_text reduce_room" value="{$data.room_num}">间
                        </div>
                ...
                <div class="yr_list">
                    ...
                    <div class="we_b" style="width:100%;text-align:center">
                        <div class="we_b2 center ft12" style="width:100%;padding:8px;">
                            <input style="text-align:center" type="text" name='people' class="we_people reduce" value="{$pcount}" readonly>人
                        </div>
                    </div>
                </div>
                <div class="yr_list">
                    ...
                    <div class="we_c">
                      <volist name="partners" id="partner">
                        <div class="name_list" id="partner_{$partner.phone}">
                        <div class="name_text">{$partner.realname}</div>
                        <input type="hidden" name="link_id" value="{$partner.lmid}">
                        <div class="name_a">
                        <input class="partners" type="hidden" name="partners[]" value="{$partner.realname},{$partner.phone},{$partner.idcard}">
                        <input type="button" data-name="{$partner.realname}" data-linkId="{$partner.lmid}" data-phone="{$partner.phone}" data-idcard="{$partner.idcard}" class="name_btn edit_partner" data-origin="#partner_{$partner.phone}" value="编辑">
                        <input type="button" class="name_btn del_partner" data-origin="#partner_{$partner.phone}" value="删除">
                        </div>
                        </div>
                      </volist>
                    </div>
                ...
                <div class="we_d">
                    <div class="lu_b">
                        <input type="text" class="ft12 lu_text" name="realname" placeholder="真实姓名 :" value="{$data.realname}">
                    </div>
                    <div class="lu_b">
                        <input type="text" class="ft12 lu_text" name="phone" placeholder="电话号码 :" value="{$data.phone}">
                    </div>
                    <div class="lu_b">
                        <input type="text" class="ft12 lu_text" name="idcard" placeholder="身份证号码 :" value="{$data.idcard}">
                    </div>
                </div>
            </div>
            <div class="ig" style="position:fixed;left:0;right:0;bottom:0">
                <div class="ig_left fl">
                    <div class="ig_a ft12" style="line-height:20px">订单总额 :
                        <span>
               <em class="ft14">￥</em><span class="ft16" id='total'>{$data.money}</span>
                        </span>
                    </div>
    ...
    <script>
      ...
      (function() {
        ...
        (function() {
          ...
          $('input.del_partner').click(function(evt) {
            evt.preventDefault(); 
            delPartner($($(this).data('origin')));
          });

          //将已选入住人填充入数组.
          $('.we_c').find('input.partners').each(function(i, p) {
            var _this = $(p);
            var value = _this.val();
            var arr = value.split(',');
            nameArr.push(arr[0]);
