<include file="public:head" />
<script src="__JS__/chosen.jquery.js"></script>
    <link href="__CSS__/chosen.css" rel="stylesheet" />
    <script src="__JS__/jquery-ui.min.js"></script>
    <link href="__CSS__/jquery-ui.min.css" rel="stylesheet" />
    <script src="__JS__/jquery.jqtransform.js"></script>
<include file="public:mheader" />
<div style="background:#f4f4f4;">
        <div class="wrap">
            <div style="padding-top:28px;"></div>
            <div class="payment hidden">
                <div class="fl payment_main_01">
                    <span>填写报名表</span>
                </div>
                <div class="fl payment_main_05">
                    <!--灰色样式类名 payment_main_02    蓝色样式类名payment_main_05-->
                    <span>活动审核</span>
                </div>
                <div class="fl payment_main_06">
                    <!--灰色样式类名 payment_main_03    蓝色样式类名payment_main_06-->
                    <span>支付钱款</span>
                </div>
                <div class="fl payment_main_07">
                    <!--灰色样式类名 payment_main_04    蓝色样式类名payment_main_07-->
                    <span>报名成功</span>
                </div>
            </div>
        </div>
        <div class="wrap">
            <div class="payment_main2 clearfix">
                <span class="payment_main2_span">活动确认信息 :</span>
                <div class="Landlord_confirmation_main">
                    <div class="Landlord_confirmation_main_1">
                        <div class="Landlord_confirmation_main_2">
                            <img src="__IMG__/Icon/img73.png" />
                            <span>您活动报名已经成功</span>
                            <i class="tl">活动信息 ：</i>
                        </div>
                        <div class="Landlord_confirmation_main_3 hidden">
                            <div class="fl Scheduled_finish_main1">
                                <div>
                                    <span class="f14 c999">活动时间：</span>
                                    <i class="f14 c333">2016年6月1日</i>
                                </div>
                                <div>
                                    <span class="f14 c999">活动人数：</span>
                                    <i class="f14 c333">2016年6月5日</i>
                                </div>
                                <div>
                                    <span class="f14 c999">活动地址：</span>
                                    <i class="f14 c333">杭州西湖升力客栈酒店-蓝色妖姬主题房间  </i>
                                </div>
                            </div>
                            <div class="fr Scheduled_finish_main2">
                                <span class="f14 c999">房东 ：</span>
                                <a href="">美丽的风铃</a>
                            </div>
                        </div>
                    </div>
                </div>
                <span class="Landord_confirmation_span">该活动附近的美宿推荐 :</span>
                <div class="Landord_confirmation_main6 hidden">
                    <ul class="Landord_confirmation_main6_ul">
                        <li>
                            <div class="Landord_confirmation_main6_list">
                                <div class="pr Landord_confirmation_main6_list2">
                                    <a href="">
                                        <img src="__IMG__/img68.jpg" />
                                    </a>
                                    <div class="pa Landord_confirmation_main6_list2_01">
                                        <i>￥<em>600</em></i><span>起</span>
                                    </div>
                                    <div class="Landord_confirmation_main6_list2_02 pa">
                                        <img src="__IMG__/Icon/img8.png">
                                    </div>
                                </div>
                                <div class="Landord_confirmation_main6_list_list3">
                                    <a href="">哈喽哈喽的粉色主题房间</a>
                                    <div class="hidden">
                                        <div class="fl">
                                            <img src="__IMG__/Icon/img10.png" />
                                            <span class="f14 c999"><em>188</em>条点评</span>
                                        </div>
                                        <div class="fr tr cur">
                                            <img src="__IMG__/Icon/img9.png" />
                                            <i class="f12 c999">28</i>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </li>
                        <li>
                            <div class="Landord_confirmation_main6_list">
                                <div class="pr Landord_confirmation_main6_list2">
                                    <a href="">
                                        <img src="__IMG__/img68.jpg" />
                                    </a>
                                    <div class="pa Landord_confirmation_main6_list2_01">
                                        <i>￥<em>600</em></i><span>起</span>
                                    </div>
                                    <div class="Landord_confirmation_main6_list2_02 pa">
                                        <img src="__IMG__/Icon/img8.png">
                                    </div>
                                </div>
                                <div class="Landord_confirmation_main6_list_list3">
                                    <a href="">哈喽哈喽的粉色主题房间</a>
                                    <div class="hidden">
                                        <div class="fl">
                                            <img src="__IMG__/Icon/img10.png" />
                                            <span class="f14 c999"><em>188</em>条点评</span>
                                        </div>
                                        <div class="fr tr cur">
                                            <img src="__IMG__/Icon/img9.png" />
                                            <i class="f12 c999">28</i>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </li>
                        <li>
                            <div class="Landord_confirmation_main6_list">
                                <div class="pr Landord_confirmation_main6_list2">
                                    <a href="">
                                        <img src="__IMG__/img68.jpg" />
                                    </a>
                                    <div class="pa Landord_confirmation_main6_list2_01">
                                        <i>￥<em>600</em></i><span>起</span>
                                    </div>
                                    <div class="Landord_confirmation_main6_list2_02 pa">
                                        <img src="__IMG__/Icon/img8.png">
                                    </div>
                                </div>
                                <div class="Landord_confirmation_main6_list_list3">
                                    <a href="">哈喽哈喽的粉色主题房间</a>
                                    <div class="hidden">
                                        <div class="fl">
                                            <img src="__IMG__/Icon/img10.png" />
                                            <span class="f14 c999"><em>188</em>条点评</span>
                                        </div>
                                        <div class="fr tr cur">
                                            <img src="__IMG__/Icon/img9.png" />
                                            <i class="f12 c999">28</i>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </li>
                        <li>
                            <div class="Landord_confirmation_main6_list">
                                <div class="pr Landord_confirmation_main6_list2">
                                    <a href="">
                                        <img src="__IMG__/img68.jpg" />
                                    </a>
                                    <div class="pa Landord_confirmation_main6_list2_01">
                                        <i>￥<em>600</em></i><span>起</span>
                                    </div>
                                    <div class="Landord_confirmation_main6_list2_02 pa">
                                        <img src="__IMG__/Icon/img8.png">
                                    </div>
                                </div>
                                <div class="Landord_confirmation_main6_list_list3">
                                    <a href="">哈喽哈喽的粉色主题房间</a>
                                    <div class="hidden">
                                        <div class="fl">
                                            <img src="__IMG__/Icon/img10.png" />
                                            <span class="f14 c999"><em>188</em>条点评</span>
                                        </div>
                                        <div class="fr tr cur">
                                            <img src="__IMG__/Icon/img9.png" />
                                            <i class="f12 c999">28</i>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
<include file="public:foot" />