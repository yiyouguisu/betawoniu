...
<div class="wrap">
        ...
        <div class="Snail_home_main hidden">
           ...
           <div class="fl Snail_home_mr">
                <div class="My_message_details_m">
                    ...
                    <div class="My_message_details_x">
                        <span>退订理由：</span>
                        <p>{$order.content}</p>
                    </div>
                </div>
                ...
                <div class="My_message_details_m3 hidden">
                    <input class="adopt" type="button" value="同意退订" data-orderid="{$order.orderid}" />
                    <input class="Not_through Event_details5_6_list3_03" type="button" value="不同意退订" data-orderid="{$order.orderid}" />
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="Popup">
        ...
        <div class="Popup3_Amount hide">
            <div class="Popup_top pr">
                <span>请输入退订金额</span>
                <img class="img_hide2" src="__IMG__/Icon/img107.png" />
            </div>
            <div class="Popup3_Amount_bottom">
                <div class="Popup3_Amount_count">
                    <span>退订金额 :</span>
                    <div class="Popup3_Amount_text">
                        <i>￥</i>
                    ...
                    <div class="Popup_sub">
                        <input type="hidden" name="orderid" value="">
                        <input class="sub_z" type="button" value="确定提交" />
                    </div>
                </div>
            </div>
        </div>
    ...
    <script type="text/javascript">
        $(function () {
            $(".sub_z").live("click",function () {
                ...
                $.post("{:U('Home/Order/ajax_refundreview')}",{"orderid":orderid,"uid":uid,"money":money,"status":2},function(d){
                    d=eval("("+d+")");
                    if(d.code==200){
                        alert(d.msg)
            ...
            $(".adopt").click(function () {
                $(".Popup3_Amount,.Mask_x").show();
                $("html,body").css({
                    "overflow-y": "hidden"
                })
                $("input[name='orderid']").val($(this).data("orderid"));
            })
            $(".Mask_x,.img_hide2").click(function () {
                $(".Mask_x,.Popup3_Amount").hide();
                $("html,body").css({
                    "overflow-y": "scroll"
                })
            })
            // $(".Not_through").live("click",function(){
            //     var uid="{$user.id}";
            //     if(uid==''){
            //         alert("请先登录！");var p={};
            //         p['url']="__SELF__";
            ...
            //     $.post("{:U('Home/Order/ajax_refundreview')}",{"orderid":orderid,"uid":uid,"status":3},function(d){
            //         d=eval("("+d+")");
            //         if(d.code==200){
            //             $(".Mask_x,.Popup3_Amount").hide();
            //             $("html,body").css({
            ...
            $("#reportsave").live("click",function(){
                ...
                var orderid=$("input[name='orderid']").val();
                $.post("{:U('Home/Order/ajax_refundreview')}",{"orderid":orderid,"remark":remark,"uid":uid,"status":3},function(d){
                    d=eval("("+d+")");
                    if(d.code==200){
                        $(".My_message_details_main2").hide();
                        $("html,body").css({
