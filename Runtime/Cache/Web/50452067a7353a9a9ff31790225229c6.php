<?php if (!defined('THINK_PATH')) exit(); if(is_array($party)): $i = 0; $__LIST__ = $party;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="recom_list pr">
      <div class="recom_a pr">
          ...
          <a href="<?php echo U('Web/Member/memberHome',array('id'=>$vo['uid']));?>">
            <div class="recom_d pa"><img src="<?php echo ($vo["head"]); ?>" style="width:60px;height:60px"></div>
          </a>
      </div>
      <div class="recom_c pa">
        <div class="recom_gg collect <?php if($vo["iscollect"] == 1): ?>recom_c_cut<?php endif; ?>" data-id="<?php echo ($vo["id"]); ?>" data-uid="<?php echo ($uid); ?>" <?php if($vo["iscollect"] == 1): ?>data-collect="1" <?php else: ?> data-collect="0"<?php endif; ?> ></div>
      </div>
      <div class="recom_e">
        ...
        <div class="recom_s f16">
            已参与：
            <span id="sapn">
                <?php if(is_array($vo['joinlist'])): $i = 0; $__LIST__ = $vo['joinlist'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><img src='<?php echo ($v["head"]); ?>' style="width:40px;height:40px;"><?php endforeach; endif; else: echo "" ;endif; ?>
            </span>
