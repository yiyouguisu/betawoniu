...
class AutoPushController extends CommonController {
	...
	public function push(){
		$strOut="";
		$push_queue = M('sendpush_queue')->field('id,mid,receiver,title,description,extras')->limit(50)->where(array('status'=>0))->select();
		foreach($push_queue as $v){
			...
			$res_arr=self::pushMessage($v['description'],$v['title'],$receiver,unserialize($v['extras']));
            M('pushlog')->add(array(
                'receiver'=>$v['receiver'],
                'title'=>$v['title'],
                'extras'=>$v['extras'],
	...
	public function pushMessage($message='',$title='',$receiver='all',$extras=array(),$message_type=1,$m_time='86400',$platform='all'){
