...
class PublicController extends Controller {
    ...
    public function storehouse() {
        ...
        $data = M("order_productinfo a")
            ...
            ->order(array('b.isspeed'=>'desc','c.distribute_time'=>'desc'))
            ->group("a.orderid")
        ...
        foreach ($data as $key => $value)
        {
        	$data[$key]['productinfo']=M("order_productinfo a")
                ->join("left join zz_product d on a.pid=d.id")
                ->where(array('a.orderid'=>$value['orderid'],'d.storehouse'=>$sid))
                ->field('a.pid,d.title as productname,a.nums,d.unit,a.product_type,d.standard')
                ->select();
        }
